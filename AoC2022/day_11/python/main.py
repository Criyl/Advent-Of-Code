...
class Monkey:
    ...
    def __init__(
        self, monkeyID, initialInventory, operation, check, trueMonkey, falseMonkey
    ):
    ...
    def create(text, withLCM=False):
        ...
        inventory = list(map(lambda x: int(x), inventory.split(", ")))

        groups = re.match(
            r"  Operation: new = (old|\d+) ([+*]) (old|\d+)", lines[2]
        ).groups()
        ...
        trueMonkey = re.match(r"    If true: throw to monkey (\d+)", lines[4]).groups()[
            0
        ]
        falseMonkey = re.match(
            r"    If false: throw to monkey (\d+)", lines[5]
        ).groups()[0]
